...
public class MsgHandlerThread extends HandlerThread {
    ...
    int dis_pos = 0;
    static int sHANDLES;
    ArrayList<ReqTuple> mXmitQueue = new ArrayList();
    ArrayList<ReqTuple> mPendingXmitQueue = new ArrayList();
                    ...
                    switch (msg.what) {
                        ...
                        case WQREQUEST:
                            ...
                            if (MsgHandlerThread.this.mPendingXmitQueue.size() > 0) {
                                MsgHandlerThread.this._sendNext((ReqTuple) MsgHandlerThread.this.mPendingXmitQueue.remove(0));
                            }
                        ...
                        case WQRESPONSE:
                            ...
                            ReqTuple rxReqTuplex = null;
                            Iterator var10 = MsgHandlerThread.this.mXmitQueue.iterator();

                            while(var10.hasNext()) {
                                ReqTuple rt = (ReqTuple)var10.next();
    ...
    private void _sendNext(ReqTuple rt) {
        this._sendRequest(rt);
    }

    private void _sendRequest(ReqTuple rt) {
        if(this.mBusy.get()) {
    ...
    protected void sendRequest(BaseRequest request, ResponseHandler sh, int timeout) {
        ...
        ReqTuple rt = new ReqTuple(request, sh, timeout);
        this._sendRequest(rt);
