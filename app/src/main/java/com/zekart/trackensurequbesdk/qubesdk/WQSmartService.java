...
public class WQSmartService extends Service {
    ...
    private void onData(String action, BluetoothGattCharacteristic characteristic) {
        ...
        if (serviceUUID.compareTo(WQSmartUuid.OBD_SERVICE.getUuid()) == 0
                && characteristicUUID.compareTo(WQSmartUuid.OBD_MEASUREMENT.getUuid()) == 0) {
            ...
            if(currentRequest.requestId == BaseRequest.REQUEST_DEVICE_ADDRESS
                    && serviceUUID.compareTo(WQSmartUuid.OBD_SERVICE.getUuid()) == 0
                    && characteristicUUID.compareTo(WQSmartUuid.OBD_DEVICE_ADDRESS.getUuid()) == 0
                    ) {
                byte[] values = characteristic.getValue();
                if(values.length == 6 ) {
                    String address = "";
